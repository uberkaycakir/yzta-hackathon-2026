...
import { Sparkles, Mail, Lock, User, Store, ArrowLeft } from 'lucide-react';

const Register = ({ onRegister, onBack, onGoToLogin }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onRegister(formData.name || 'Kullanıcı');
  };
    ...
    <div className="auth-page">
      ...
      <div className="auth-card glass">
        <div className="auth-header">
          ...
          <p>Mağazanızı dijitalleştirmek için ilk adımı atın.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <div className="form-group">
            <label><Store size={16} /> Mağaza Adı</label>
            <input 
              type="text" 
              required 
              value={formData.storeName}
              onChange={(e) => setFormData({...formData, storeName: e.target.value})}
              placeholder="Yılmaz Süpermarket" 
            />
          </div>
          <div className="form-group">
          ...
          <button type="submit" className="primary-btn full-width">Kayıt Ol ve Başla</button>
        </form>

        <div className="auth-footer">
          Zaten hesabınız var mı? <button className="link-btn" onClick={onGoToLogin}>Giriş Yap</button>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .auth-form .form-group label { display: flex; align-items: center; gap: 0.5rem; font-weight: 700; margin-bottom: 0.4rem; font-size: 0.9rem; }
        .auth-form input { 
